...
def register_user():
    data = UserRegistrationSchema().load(request.json)
...
def update_user_name(current_user):
    data = UserRegistrationSchema(only=['name']).load(request.json)
...
def update_user_email(current_user):
    data = UserRegistrationSchema(only=['email']).load(request.json)
...
def update_user_password(current_user):
    data = UserRegistrationSchema(only=['password']).load(request.json)
